...
Shader "Custom/DynamicSurfaceKinect2Shader" {
	Properties {		
		...
		txBackground("txBackground", 2D) = "white" { }		
        WorldScale("WorldScale", Float) = 1
		PlaneZOffset("PlaneZOffset", Float) = 0
	...
	SubShader {
		...
		Texture2D<float> txBackground;
		float4 txBackground_ST;
		...
		void GS(point GS_INPUT particles[1], uint primID : SV_PrimitiveID, inout TriangleStream<PS_INPUT> triStream)
		{
			...
			for (int subquad = 0; subquad< 4; ++subquad){
			    ...
			    for (int c = 0; c < 4; ++c){
					...
					realDepthsBackground = txBackground.Load(uv);
			    }
				...
				//float4 avgDepthBackground = (realDepthsBackground[0] + realDepthsBackground[1] + realDepthsBackground[2] + realDepthsBackground[3]) / 4.0;
			    
				static const float backCutoff = 130.0;
				float4 depthDevb = abs(realDepths - realDepthsBackground);
				float4 branchd = step(backCutoff, depthDevb);
			    ...
			    static const float joinCutoff = 60.0;
			    ...
			    {      
					...
					WorldPos.xyz = WorldPos.xyz * WorldScale;

					WorldPos.z -= PlaneZOffset;
