...
Shader "Custom/UbitrackARShaderDifferentCameraVertex" {
      Properties
	   {	   	    
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : COLOR
            {
             	...
             	float4 pos_pref = mul(_WebPV_Pref, i.position_in_world_space);
				float4 uv_pref = ComputeScreenPos (pos_pref);
				
				float4 pos_other = mul(_WebPV_Other, i.position_in_world_space);
				float4 uv_other = ComputeScreenPos (pos_other);
				
	            float sceneZ_pref = uv_pref.z;
	            float sceneZ_other = uv_other.z;
	         
				
				uv_pref = uv_pref / uv_pref.w;
				uv_other = uv_other / uv_other.w;
				
		
	              
				
			
				float2 uv_pref_image = TRANSFORM_TEX (uv_pref.xy, _WebCam_Pref_Image);  		
				float4 color_pref = tex2D (_WebCam_Pref_Image, uv_pref_image);
				
				float2 uv_other_image = TRANSFORM_TEX (uv_other.xy, _WebCam_Other_Image);  		
				float4 color_other = tex2D (_WebCam_Other_Image, uv_other_image);
 				
 				
				
				float2 uv_pref_depth = TRANSFORM_TEX (uv_pref.xy, _WebCam_Pref_Depth);  				
				float4 pref_depth = tex2D (_WebCam_Pref_Depth, uv_pref_depth);
				
				float2 uv_other_depth = TRANSFORM_TEX (uv_other.xy, _WebCam_Other_Depth);  	
				float4 other_depth = tex2D (_WebCam_Other_Depth, uv_other_depth);
				
				  //Actual distance to the camera
                float zCamPref = pref_depth.z;
                float zCamOther = other_depth.z;
            
               
 
                //If the two are similar, then there is an object intersecting with our object
                 float diffPref = abs(sceneZ_pref - zCamPref);
                 float diffOther = abs(sceneZ_other - zCamOther);
